        ...
        command.CommandText = """
            SELECT NOT r.rolsuper AND NOT r.rolbypassrls AND NOT r.rolcreaterole AND NOT r.rolcreatedb
              AND NOT EXISTS (
                ...
                WHERE n.nspname IN ('identity', 'operations', 'communications') AND c.relowner = r.oid)
              AND NOT EXISTS (
                SELECT 1 FROM pg_namespace n WHERE n.nspname IN ('identity', 'operations', 'communications') AND n.nspowner = r.oid)
