...
public sealed class TimelineEntry : TenantEntity
{
    ...
    public static TimelineEntry From(VendorAssigned e) => WorkEvent(
        ...
        e.PreviousVendorId?.ToString(), e.VendorId.ToString(), e.EventId);
    ...
    public static TimelineEntry From(EmployeeAssigned e) => WorkEvent(
        ...
        e.PreviousEmployeeId?.ToString(), e.EmployeeId.ToString(), e.EventId);
    ...
    public static TimelineEntry From(WorkReopened e) => WorkEvent(
        ...
        e.PreviousStatus.ToString(), e.NewStatus.ToString(), e.EventId);
    ...
    public static TimelineEntry WorkEvent(Guid organizationId, Guid actorId, DateTimeOffset occurredAt,
        string eventType, Guid workId, string? oldValue, string? newValue, Guid? id = null) => Record(
        organizationId, actorId, occurredAt, eventType, "WorkItem", workId, oldValue, newValue, workId,
        JsonSerializer.Serialize(new { oldValue, newValue }), id);
        ...
        Guid? workId = null, string changes = "{}", Guid? id = null, bool residentVisible = false)
    {
        ...
        if (actorId == Guid.Empty || workId == Guid.Empty || relatedObjectId == Guid.Empty || id == Guid.Empty) throw new ArgumentException("Identifiers cannot be empty.");
        ...
        return new TimelineEntry(organizationId, id ?? Guid.NewGuid())
        {
