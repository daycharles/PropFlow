...
- **Attention** — `AttentionRules` (pure domain) evaluates seven rules over every open work item; `GET /api/attention` returns the findings most-urgent-first with per-severity distinct-work counts (PF-6.06 `#60`). The `/attention` screen shows Critical / Warning / Informational cards that each filter the list, every row linking to its work order (PF-6.07 `#61`).
