...
Every timeline row has one shape, whatever produced it: event type, optional actor, UTC timestamp, an old/new value pair, a `changes` JSON blob holding the same pair, and an optional related-object reference (`RelatedObjectType`, `RelatedObjectId`). There are no event-specific columns — the legacy `PreviousVendorId`/`VendorId` pair was dropped in `20260910122538_TimelineDropLegacyVendorColumns`. `WorkId` is optional, so an entry can hang off a work item by related-object reference alone; the work timeline read returns both. Domain events keep their own typed fields (`VendorAssigned` carries `PreviousVendorId` and `VendorId`, `WorkReopened` carries the two statuses); the entry a domain event produces takes the event's ID as its primary key, so a replayed event cannot append a second row.

No-op assignments create no duplicate history. Assignment plus timeline insertion commit in one SaveChanges transaction. Timeline modifications/deletions are blocked by EF, runtime database permissions and a PostgreSQL trigger. No external communication leaves the system: the milestone-4 outbox and dispatcher below are live and `POST /api/work/{id}/message` enqueues a resident message against a work item (`CommunicationEndpoints.cs:130`), but the only registered senders are the recording mocks (`src/PropFlow.Api/Program.cs:68-69`), so nothing is delivered off-box. No *work event* enqueues a row on its own yet — the enqueue is an explicit operator action.
...
Milestone 6 also adds, in the `operations` schema, the `Asset` record (type, make/model/serial, install date, warranty, service life, condition, replacement cost) and its optional link from a `WorkItem` (`AssetId`, composite FK, same-property enforced in the application layer). A per-organization `RepeatRepairPolicy` table — one row per tenant, forced RLS and `tenant_isolation` like every business table — holds the repeat-repair thresholds (default 3 repairs / 120 days, optional category-similarity); `IRepeatRepairDetector` counts published work linked to an asset inside the window, and the pure `AttentionRules` evaluates seven triage rules (unassigned emergency, first-response SLA breach, overdue, waiting on vendor/resident, repeat repair, unit-turn at risk) over every open work item for the `/api/attention` queue. `GET /api/search` is a `pg_trgm` fuzzy search across the tenant-scoped operations tables. These read surfaces are unpaged today (see [followups.md](followups.md)).
...
- Work scope lives in `WorkAccessScope` (PF-5.01/5.02, shipped): regional access requires an explicit property grant; technician and vendor access require the current matching employee/vendor assignment, and an optional property allow-list can only narrow that assignment. An empty scope denies, and the work endpoints filter against it.
