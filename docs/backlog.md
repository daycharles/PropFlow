...
| PF-4.09 | ✅ Mobile-responsive Work list and detail; large touch targets for field use. `box-sizing` reset + one `@media (max-width: 700px)` breakpoint: the Work list `<table>` becomes labelled cards, column-header sorting → a `.mobile-sort` select, controls `min-height: 44px`. `e2e/mobile.spec.ts`. | web | M | PF-3.19 |
