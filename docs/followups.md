...
| `WorkItem.AssignVendor` / `AssignEmployee` / `Schedule` / `Edit` don't guard terminal states | code audit 2026-09-09; `src/PropFlow.Domain/Work/WorkItem.cs:52-57` | daycdev | `ChangeStatus` enforces the terminal-state guard; the four above skip it, so a `Completed` or `Cancelled` work item can still be re-assigned, re-scheduled or edited. Surfaced by PF-3.22: the demo seed now contains terminal rows, and the e2e select-all flow has to filter to `Status=New` to avoid assigning a vendor to them. `CreatedAt` is also only set in `Publish`, so a `Draft` row carries `default(DateTimeOffset)` |
