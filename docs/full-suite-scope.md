...
### FS-G3B — Operations workflows (#187)

**FS-S11 — Maintenance and work-order operations baseline (#199).** The existing work-order
surface remains the system of record for assignment, scheduling, resident communication, timeline,
automation, assets, attachments, attention queues, and technician workflows. Cross-module links
are auditable through the work timeline and the composite tenant foreign keys to properties,
vendors, employees, assets, and attachments. The regression suite is the acceptance gate; no
parallel replacement work-order aggregate is introduced.

**FS-S12 — Inspections, turns, and make-ready (#200).** Inspection templates are versioned and
archivable. Inspections move through draft/in-progress/completed/approved states; findings have
an explicit open/resolved/waived lifecycle and may reference existing tenant-scoped attachment
ids for photos. A completed or approved move-out inspection can generate a unit turn and one
linked `WorkItem` per open finding, preserving the existing work-order audit, assignment, due-date,
resident/owner visibility, and tenant-isolation controls. A turn cannot become ready until every
generated task is complete. Read-only callers can inspect but cannot create or mutate templates,
inspections, findings, turns, or tasks.

The S11/S12 API is under `/api/inspection-templates`, `/api/inspections`, and `/api/unit-turns`.
The shared OperationsStore migration is intentionally a merge point with the in-flight S14
