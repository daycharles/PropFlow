...
**Deployment posture.** There is no container image for the application and no hosted environment. Two supported paths, both single-host:

- **[docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)** — the production-posture path. Download a release bundle and run `./propflow-deploy up`: one self-contained executable that preflights the machine, starts PostgreSQL, migrates, configures the restricted runtime role, seeds, builds the web app and starts everything with `ASPNETCORE_ENVIRONMENT=Production`. No .NET SDK needed on the target. Build the bundles with `./scripts/Publish-Deployment.ps1`.
- **[docs/RELEASE-TESTING.md](docs/RELEASE-TESTING.md)** — running from source in Development, with `dotnet run` and `next dev`.
